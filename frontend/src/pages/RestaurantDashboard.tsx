...
export default function RestaurantDashboard() {
  ...
  const [activePage, setActivePage] = useState<'dashboard' | 'menu' | 'orders'>('dashboard');
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`appshell-nav-item w-full flex items-center justify-between px-4 py-3 rounded-xl text-xs font-semibold transition-all duration-200 group cursor-pointer ${
                  isActive
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-1.5 rounded-lg transition-colors ${
                    isActive ? 'bg-white/10 text-white' : 'bg-muted text-muted-foreground group-hover:bg-card group-hover:text-foreground'
                  }`}>
                    {item.icon}
                  </div>
                  <span className="appshell-sidebar-label text-left">{item.label}</span>
                </div>
                {item.badge && <span className="appshell-sidebar-label">{item.badge}</span>}
              </button>
            );
          })}
        </div>
